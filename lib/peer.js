...
function Peer(id, metadata, options, connection, node) {
  var self = this;

  if (!(this instanceof Peer)) {
    return new Peer(id, metadata, options, connection, node);
  }
...
P.connect = function connect() {
  var connection = this.transport.connect(
    this.node.id, this.node.metadata, this.id, this.metadata);
  this.setupConnection(connection);
  return connection;
};
...
P._invoke = function _invoke(type, args, cb) {
  ...
  function onTimeout() {
    timeout = undefined;
    var err = new Error(
      'invoke timeout after ' + self.node.options.commandTimeout + ' ms');
    err.timeout = true;
    callback(err);
  }
};
