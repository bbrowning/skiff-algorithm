...
R.replicateToPeer = function replicateToPeer(peerId) {
  ...
  if (peer.nextIndex <= log.lastIncludedIndex &&
      this.node.commonState.volatile.commitIndex > 0)
  {
  ...
  function replied() {
    ...
    args = ['response', peer.meta.id, index, entries.length].concat(args);
